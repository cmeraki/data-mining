...
'''def chunk_based_on_rules(captions):
    ...
    while i < len(captions):
        ...
        chunk_end = cumulative_start + max_duration

        # Check if current caption causes overflow
        if (start_ms <= chunk_end <= end_ms or end_ms - cumulative_start > max_duration):
            if cumulative_text:
                intervals.append({
                    'start': cumulative_start,
                    'end': chunk_end,
            ...
            cumulative_start = chunk_end
            cumulative_text = ""
        elif caption['text'][-1] in ['.', '?', '!', ';']:
            line_end = end_ms
            if line_end - cumulative_start >= min_duration:
                intervals.append({
                    'start': cumulative_start,
                    'end': line_end,
                    'text': cumulative_text.strip()
            ...
            line_end = end_ms
            if line_end - cumulative_start > max_duration:
                intervals.append({
                    'start': cumulative_start,
                    'end': line_end,
                    'text': cumulative_text.strip()
